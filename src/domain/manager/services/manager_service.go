...
import (
	...
	repository "github.com/DevShuxat/logistic-staff-service/src/domain/manager/repositories"
...
func (s *managerSvcImpl) LoginManager(ctx context.Context, email, password string) (*models.ManagerProfile, error) {
	...
	if !crypto.PasswordMatch(password, manager.PasswordSalt, manager.PasswordHash) {
		...
		profile, err := s.managerRepo.GetManagerProfile(ctx, manger.ID)
		if err != nil {
			return nil, err
		}
		return profile, nil
...
func (s *managerSvcImpl) RemoveManagerAssignmentFromRestaurant(ctx context.Context, assignmentID int) error {
	if err := s.managerRepo.DeleteManagerRestaurantAssignment(ctx, &assignmetID); err != nil {
		return err
	}
	return nil
